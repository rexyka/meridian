...
import { evaluateExitAlertsForPositions, formatExitAlerts, summarizeExitAlertCluster } from "./tools/exit-alerts.js";
...
export async function runManagementCycle({ silent = false } = {}) {
  ...
  try {
    ...
    const reportLines = positionData.map((p) => {
      ...
      const cluster = summarizeExitAlertCluster(p.exit_alerts);
      const alertStatus = cluster.level !== "OK" ? ` | ${cluster.icon} ${cluster.label}` : "";
      const statusLabel = `${act.action === "INSTRUCTION" ? "HOLD (instruction)" : act.action}${alertStatus}`;
...
async function telegramHandler(msg) {
  ...
  if (closeMatch) {
    try {
      ...
      const result = await executeTool("close_position", { position_address: pos.position, reason: "telegram /close command" });
      if (result.success) {
        const closeTxs = result.close_txs?.length ? result.close_txs : result.txs;
        const claimNote = result.claim_txs?.length ? `\nClaim txs: ${result.claim_txs.join(", ")}` : "";
        const swapNote = result.auto_swapped ? `\nAuto-swapped base token → SOL` : "";
  ...
  if (text === "/closeall") {
    try {
      ...
      for (const pos of positions) {
        try {
          const result = await executeTool("close_position", { position_address: pos.position, reason: "telegram /closeall command" });
